...
bool ModelDetection::getFrameDetections(cv::Mat& frame, DETECTIONS& res) {
	...
	std::cout<<"width    "<<net.w<<std::endl;
	IplImage src = frame_temp;
	image im = ipl_to_image(&src);   //20ms
	start1 = clock();
	rgbgr_image(im);
	finish1 = clock();
	std::cout<<"resize    "<<finish1 - start1<<std::endl;
	...
	finish1 = clock();
	std::cout<<"network_predict    "<<finish1 - start1<<std::endl;
	int nboxes = 0;
	int letterbox = 0;
	start1 = clock();
	...
	finish1 = clock();
	std::cout<<"drawbox    "<<finish1 - start1<<std::endl;
